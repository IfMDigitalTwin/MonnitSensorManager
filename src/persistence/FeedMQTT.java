...
public class FeedMQTT extends AbstractVerticle {
    ...
    public void start(Future<Void> fut) throws Exception {
        ...
        if (!get_config()){
        	System.err.println("FeedMQTT: "+ MODULE_ID + " failed to load initial config()");
        	vertx.close();
        	return;
        }


        System.out.println(MODULE_NAME+"."+MODULE_ID+": Version "+VERSION+" started");
    ...
    private void add_feed_handler(JsonObject config)
    {
        ...
        if (feed.client == null)
        {
            System.err.println(MODULE_NAME+"."+MODULE_ID+"."+feed.FEED_ID+
  ...
  private void process_feed(MqttPublishMessage mqtt_msg, JsonObject config) throws Exception
  {
    ...
    try {
        ...
        System.err.println(MODULE_NAME+"."+MODULE_ID+": prepared EventBus msg:");
            ...
            if (monitor_result.succeeded())
            ...
            else
                {
                    System.err.println(MODULE_NAME+"."+MODULE_ID+
                                ...
                                monitor_path);
                    System.err.println(monitor_result.cause().getMessage());
                }
        });
    }
    ...
    private void write_file(FileSystem fs, Buffer buf, String file_path)
    {
        fs.writeFile(file_path,
            ...
            if (result.succeeded()) {
                ...
                System.err.println(MODULE_NAME+"."+MODULE_ID+": write_file error ..." + result.cause());
            }
        });
    } // end write_file
    ...
    private boolean get_config()
    {
        ...
        if (MODULE_NAME == null)
            {
                System.err.println("FeedMaker: config() not set");
                return false;
            }
    ...
    private class MqttFeed {
        ...
        public MqttFeed(JsonObject config) {
            ...
            if (PORT == null)
            {
                ...
                System.err.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+
        ...
        private void init(MqttClientOptions client_options)
        {
            ...
            client.publishHandler( mqtt_data -> {
                System.err.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+": MQTT data received");
                try {
            ...
            System.out.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+": MQTT handler started");
        }
        ...
        private void connect()
        {
            ...
            client.connect(PORT, HOST, connect_response -> {
                ...
                {
                    System.out.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+": MQTT connected");
                ...
                {
                    System.err.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+": MQTT connect FAILED");
                }
            }); // end connect
        }
        ...
        {
             System.out.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+": MQTT subscribing");
        ...
        {
            System.out.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+
            ...
            if (!watchdog_running)
            {
                ...
                System.out.println(MODULE_NAME+"."+MODULE_ID+"."+FEED_ID+": MQTT starting watchdog");
